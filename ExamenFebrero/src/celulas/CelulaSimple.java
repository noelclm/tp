...
public class CelulaSimple implements Celula{
	...
	public Casilla ejecutaMovimiento(Casilla casillaInicial, StringBuilder texto, MundoSimple superficie) {
		...
		if(numPosicionesVacias>0){ 
			...
			texto.append("->Celula simple en ("+filaCasillaInicial+","+columnaCasillaInicial+") se mueve a ("+filaCasillaFinal+","+columnaCasillaFinal+")"+LINE_SEPARATOR);
			...
			if (this.reproducirse()){
				...
				texto.append("->Nace una nueva celula simple en ("+filaCasillaInicial+","+columnaCasillaInicial+") cuyo padre ha sido ("+filaCasillaFinal+","+columnaCasillaFinal+")"+LINE_SEPARATOR);
			}else
				this.sumPasosDados();
			...
			if (this.muertePorInactividad()){
				texto.append("->Muere la celula simple de la casilla en ("+filaCasillaInicial+","+columnaCasillaInicial+")  por inactividad"+LINE_SEPARATOR);
			...
			// Comprobamos si ha llegado al limite de pasos que tiene que dar para reproducirse
				...
				texto.append("->Muere la celula simple de la casilla en ("+filaCasillaInicial+","+columnaCasillaInicial+")  por no poder reproducirse"+LINE_SEPARATOR);
				superficie.eliminarCelula(casillaInicial);
			}
			else{ 
				texto.append("->La celula simple en ("+filaCasillaInicial+","+columnaCasillaInicial+") no se ha podido mover"+LINE_SEPARATOR);
